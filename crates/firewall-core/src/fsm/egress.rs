...
fn find_pii(text: &str) -> Option<&'static str> {
    let regexes = PII_REGEXES.get_or_init(|| {
        vec![
            ("Phone (Intl)", Regex::new(r"\+\d{8,15}").expect("PII regex Phone (Intl) must compile")),
            ("Phone (US)", Regex::new(r"\b\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}\b").expect("PII regex Phone (US) must compile")),
            ("IPv4", Regex::new(r"\b(?:(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\b").expect("PII regex IPv4 must compile")),
            ("IPv6", Regex::new(r"\b(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}\b").expect("PII regex IPv6 must compile")),
            ("IBAN", Regex::new(r"\b[A-Z]{2}\d{2}[A-Z0-9]{11,30}\b").expect("PII regex IBAN must compile")),
            ("Credit Card", Regex::new(r"\b(?:\d{4}[- ]?){3}\d{1,4}\b").expect("PII regex Credit Card must compile")),
            ("SSN", Regex::new(r"\b\d{3}-\d{2}-\d{4}\b").expect("PII regex SSN must compile")),
            ("Email", Regex::new(r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b").expect("PII regex Email must compile")),
            ("GitHub Token", Regex::new(r"\b(?:ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9_]{36,}\b").expect("PII regex GitHub Token must compile")),
            ("AWS Key", Regex::new(r"\b(?:AKIA|A3T|AGPA|AIDA|AROA|AIPA|ANPA|ANVA|ASIA)[A-Z0-9]{16}\b").expect("PII regex AWS Key must compile")),
            ("API Key", Regex::new(r"(?i)\b(?:api[_-]?key|apikey|api[_-]?secret|secret[_-]?key)\s*[:=]\s*['\x22]?[A-Za-z0-9_\-]{16,}").expect("PII regex API Key must compile")),
            ("Bearer Token", Regex::new(r"(?i)\bbearer\s+[A-Za-z0-9_\-\.]{20,}").expect("PII regex Bearer Token must compile")),
            ("Password", Regex::new(r"(?i)\b(?:password|passwd|pwd|pass)\s*[:=]\s*['\x22]?[^\s'\x22]{4,}").expect("PII regex Password must compile")),
            // SA-077: Extended PII patterns — medical, biometric, identity, financial, secrets
            ("Medical Record", Regex::new(r"(?i)\bMRN[-\s]?\d{6,12}\b").expect("PII regex Medical Record must compile")),
            ("ICD-10 Code", Regex::new(r"(?i)\bICD[-\s]?10\b|\b[A-Z]\d{2}\.\d{1,4}\b").expect("PII regex ICD-10 Code must compile")),
            ("Passport", Regex::new(r"(?i)\bpassport\b[^A-Za-z0-9]*[A-Z]{1,2}\d{6,9}\b|\b[A-Z]{2}\d{7}\b").expect("PII regex Passport must compile")),
            ("National ID", Regex::new(r"(?i)(?:national[_\s-]?id|nid|nin)\s*[:=>]?\s*[A-Z0-9]{6,20}|<national[_-]?id[^>]*>[^<]{4,}<").expect("PII regex National ID must compile")),
            ("Biometric Hash", Regex::new(r#"(?i)(?:fingerprint|biometric|retina|iris)[_\s-]?(?:hash|id|data|template)?\s*["']?\s*[:=]?\s*["']?[A-Za-z0-9+/]{16,}|"fingerprint"\s*:\s*"[A-Za-z0-9+/]{16,}"#).expect("PII regex Biometric Hash must compile")),
            ("Bitcoin Address", Regex::new(r"\b[13][a-km-zA-HJ-NP-Z1-9]{25,34}\b").expect("PII regex Bitcoin Address must compile")),
            ("Ethereum Address", Regex::new(r"\b0x[0-9a-fA-F]{38,42}\b").expect("PII regex Ethereum Address must compile")),
            ("Private Key", Regex::new(r"(?i)\b(?:private[_-]?key|privkey|priv[_-]?key)\s*[:=]\s*['\x22]?[A-Za-z0-9+/=_\-]{32,}").expect("PII regex Private Key must compile")),
            ("Database URL", Regex::new(r"(?i)\b(?:postgres|mysql|mongodb|redis|sqlite)://[^\s]{10,}").expect("PII regex Database URL must compile")),
        ]
    });
